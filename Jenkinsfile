pipeline {
	...
	stages {
		stage('Cloning Github') {
			steps {
				git branch: 'dev', url: 'https://github.com/PaaS-TA/paas-ta-container-platform-api.git'
		...
		stage('copy config') {
			steps {
				sh 'cp /var/lib/jenkins/workspace/config/api/application.yml src/main/resources/application.yml'
			}
		}
		stage('Clean Build') {
            steps {

...
public class JwtUtil {
	...
	public String generateToken(UserDetails userDetails, AuthenticationRequest authRequest, UsersList userListByUserId) {
		...
		if (roles.contains(new SimpleGrantedAuthority("NAMESPACE_ADMIN"))) {
	...
	public boolean validateToken(String authToken) {
		try {
			...
			throw new  ExpiredJwtException(null, null, "TOKEN_EXPIRED", ex);
		}
	}
	...
	public List<SimpleGrantedAuthority> getRolesFromToken(String authToken) {
		...
		if (isClusterAdmin != null && isClusterAdmin == true) {
			roles = Arrays.asList(new SimpleGrantedAuthority("CLUSTER_ADMIN"));
		}

		if (isNamespaceAdmin != null && isNamespaceAdmin == true) {
			roles = Arrays.asList(new SimpleGrantedAuthority("NAMESPACE_ADMIN"));
		}

...
public class UsersController {
    ...
    @GetMapping("/clusters/{cluster:.+}/namespaces/{namespace:.+}/popup/users/{userId:.+}/namespacesRolesList")
    public Object getRolesListAllNamespacesAdminByUserId(@PathVariable(value = "cluster") String cluster,
                                                         @PathVariable(value = "namespace") String namespace,
                                                         @PathVariable(value = "userId") String userId,
                                                         @RequestParam(required = false, defaultValue = "0") int offset,
        ...
        if (namespace.toLowerCase().equals(Constants.ALL_NAMESPACES)) {
            if (isAdmin) {
                return usersService.getRolesListAllNamespacesAdminByUserId(cluster, namespace, userId, offset, limit, orderBy, order, searchName);
            } else {
                return Constants.FORBIDDEN_ACCESS_RESULT_STATUS;
            }
        }
